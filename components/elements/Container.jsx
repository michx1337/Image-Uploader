  ...
  <>
   ...
   <main className="flex h-screen flex-col items-center justify-center overflow-hidden bg-[#101827] antialiased">
    ...
    <nav className="fixed top-0 left-0 right-0 z-20 w-full p-2">
     {session ? (
      <div className="ml-auto float-right flex items-center">
      <h1 className="text-slate-400 text-xl px-3">
       Signed in as <span className="italic">{ session.user.name}</span>
      </h1>
      <button onClick={() => signOut()} className={`ml-auto flex items-center gap-2 cursor-pointer rounded-md border-0 bg-button-action-primary py-2 px-4 text-sm font-semibold text-white duration-200 hover:bg-button-action-hover `}>
      Logout
      </button>
      </div>
      ) : (
       <div className="ml-auto float-right flex items-center">
       <button onClick={() => signIn()} className={`ml-auto flex items-center gap-2 cursor-pointer rounded-md border-0 bg-[#2869FF] py-2 px-4 text-sm font-semibold text-white duration-200 hover:bg-[#2355c8]`}>
